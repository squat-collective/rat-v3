...
3. **State**: constrain `key`/`prefix` grammar (no traversal); `state.Put` outcome tri-state (COMMITTED/CONFLICT/UNKNOWN — replaces `committed:bool`); make linearizable-CAS a conformance gate + resolve the DynamoDB-vs-linearizability contradiction. ← **DO NEXT**
