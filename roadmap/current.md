...
**Immediate next concrete step:** ✅ **The contract-de-risking spike is COMPLETE** — `core/{manifest,registry,gateway,composition,arrowticket}` + CI (`make core-test`, now in `verify`; `make breaking` vs `main`). All `go test`-green: **C5** (enforced from manifests, audited — C4), **C1** (crash-mid-strategy recovers, no double-apply), **C3** (the gateway bounds a hung provider by the soft deadline), **D2** (ArrowStream ticket = TTL/single-use/bound). **🔑 The frozen wire held — no freeze-reopen** (`make breaking` confirms the spike touched no contract). Exit report: **[reviews/10](../reviews/10-phase-1-spike-exit.md)**. **➡️ The ball is now in Tom's court:** the spike de-risked the *contracts*; the deferred **12–18mo commitment-gate decision** ([ADR-013](../docs/architecture/adrs/013-phase-1-spike-and-commitment-gate.md)) — **commit** to the full core / **continue exploratory** / **redirect** — is the next call, informed by reviews/10 (+ the still-owed v2-vs-v3 answer **Q01** / external review **Q02**). The **full Phase-1 build** (D1 real isolation, D3 storage-cred, D4 attestation-enforced, C4 terminal audit, sre#4 backoff) follows that decision. Keep the freeze **local/unpushed** until C5 passes against real (non-fake) providers.
