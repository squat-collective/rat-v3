...
**Immediate next concrete step:** ✅ **ADR-016 opened** (the launch model: the core *launches* plugins via the deployment-runtime axis + dials the returned endpoint — it no longer dials pre-running providers). **Now building D1 on `phase-1`:** (1) a Go `local-process` deployment-runtime (`core/deploymentruntime`) that execs a plugin binary as an isolated child process + **enforces the I9 minimum** (refuse below non-root / cap-drop / no-new-privs → `FAILED_PRECONDITION`) + Healthcheck/Terminate; (2) a core **supervisor** that turns manifests → `Launch` → healthcheck → dial → register (the ADR-014 registry/gateway interfaces unchanged); (3) promote the in-test catalog/format fakes to small standalone binaries the runtime execs; (4) re-run composition-on-Go through **launched** providers (distinct PIDs), C5 still enforced. Then the **podman** runtime for the full profile (read-only-fs / metadata-egress) = **D1 complete**. CI (`make core-test` + `make breaking`) from commit 1; **schedule Q02** external review; keep the freeze **local/unpushed**.
