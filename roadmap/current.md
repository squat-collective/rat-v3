...
> Updated: 2026-06-01 (🎉 **Phase 0 contract surface COMPLETE** — all 18 axes frozen `v1` (`rat/1`→`rat/1.4`), 32 references conform (`make conformance` 32/32), cross-axis composition green, a 5-agent board review done ([reviews/08](../reviews/08-post-freeze-board-review.md)), and its one V2-regret fixed + `rat/1` re-cut (`0e81314`). **Now: COMPLETE & SEAL Phase 0** — close the four remaining gaps → cut a complete contract **`v1.1`** → then start **Phase 1 (the core)**. **Close-out items (1) catalog commit-linkage, (2) manifest schema freeze, and (3) the doc tail are DONE ([ADR-010](../docs/architecture/adrs/010-catalog-commit-linkage.md), [ADR-011](../docs/architecture/adrs/011-manifest-schema-freeze-and-per-kind-layer.md), reviews/08 E1/E3/E4/E7); only (4) the `v1.1` cut remains.**)
...
**Immediate next concrete step:** item (4) — **cut contract `v1.1`.** The surface is now sealed: catalog commit-linkage closes the pipeline loop, the manifest is frozen + per-kind-validated, all 18 axes are documented, and the doc tail is clear. Tag the commit `rat/1.1` (the additive surface on top of `rat/1`), record it in `done.md`, and update the contract status to "v1.1 sealed." *(Optionally fold in the cheap additive crash-safety fields — `idempotency_key` on the strategy write-leg, `ArrowStream` terminator — see [backlog.md](backlog.md) C1/C2; they're additive so they can also wait for Phase 1.)* Then **Phase 1 (the core)** begins, with the board's C1–C5/D1–D5 as its acceptance criteria. *(Items (1)–(3) are done — see above.)*
