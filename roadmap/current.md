...
> Updated: 2026-05-31 (Round 1 COMPLETE — all 6 axes, 12 inmemory refs. ROUND 2: 3 of 6 axes have a divergent real backend — `state`=sqlite, `storage`=local-fs, `catalog`=sqlite — each passing the shared vectors + a backend-specific semantic test. Remaining: format/engine/runtime. ADR-007 + ADR-008 decided AND migrated.)
...
1. **Round 2 — continue the technologically-divergent real backends** (see [backlog.md](backlog.md)). DONE: **`state`=sqlite ✅** (durability + linearizable CAS), **`storage`=local-fs ✅** (path containment + tenant isolation), **`catalog`=sqlite ✅** (durable branches/ledger + concurrent-merge safety). Remaining (the Arrow-heavy / execution ones): **`engine`=duckdb** (real SQL → Arrow; a `pip install duckdb`), **`format`=parquet** (real files via pyarrow), **`runtime`=subprocess/container** (real isolated execution). Each reuses the SAME shared golden vectors + a backend-specific semantic test. The `format`/`engine` real backends subsume the **typed-Arrow** pass (a real backend forces the real Arrow data leg, retiring the in-process stream-registry stand-in).
