...
> Updated: 2026-05-31 (Round 1 COMPLETE. ROUND 2: 4 of 6 axes have a divergent real backend — `state`=sqlite, `storage`=local-fs, `catalog`=sqlite, `runtime`=subprocess — each passing the shared vectors + a backend-specific semantic test. Remaining `format`+`engine` need conformance-vector rework first (real Arrow/SQL). ADR-007 + ADR-008 decided AND migrated.)
...
1. **Round 2 — 4 of 6 done** (see [backlog.md](backlog.md)): **`state`=sqlite ✅**, **`storage`=local-fs ✅**, **`catalog`=sqlite ✅**, **`runtime`=subprocess ✅**. **Remaining: `format` + `engine` — and these are NOT drop-in** like the other four. Their round-1 conformance data is provider-SPECIFIC, so a real backend can't just join the shared vectors:
   ...
   These two are the real "heavy" round-2 items and want a design decision before building. **The four cheap+high-value real backends are in.**
