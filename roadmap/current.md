...
**Immediate next concrete step:** ✅ **D1 step 1 — the `local-process` deployment-runtime — DONE** (`core/deploymentruntime`): launches a plugin binary as an isolated child process (distinct PID), enforces the I9 minimum (refuse below non-root / cap-drop / no-new-privs + refuse-as-root → `FAILED_PRECONDITION`), Healthcheck (PID + readiness) + Terminate; tested end-to-end via a standalone `stateplugin` binary (commit `c638202`, `make core-test` green). **Next on `phase-1`:** (2) a core **supervisor** — manifests → `Launch` → healthcheck → dial → register (ADR-014 registry/gateway unchanged); (3) promote the in-test catalog/format fakes to standalone binaries; (4) re-run composition-on-Go through **launched** providers (distinct PIDs), C5 still enforced. Then the **podman** runtime for the full profile (read-only-fs / metadata-egress) = **D1 complete**. CI (`make core-test` + `make breaking`) green; **schedule Q02** external review; keep the freeze **local/unpushed**.
