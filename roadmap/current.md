...
**Definition of done** (the board's exit criteria — [reviews/10](../reviews/10-phase-1-spike-exit.md)): **✅ C5** (extended to real providers — DONE) · **✅ C4** terminal audit incl. denials (DONE) · **C3** (add the no-deadline idle-timeout backstop) · **✅ D1** a real *enforcing* deployment-runtime (podman, full I9 profile — DONE) · **D2** (wire into the real bulk leg) · **D3** storage-cred isolation · **D4** conformance-attestation *enforced* · **C1** (against real backends) · **sre#4** reconciler crash-loop backoff/jitter.

**Immediate next concrete step:** ✅ **C5 + C4 + D1 DONE.** **C5** is enforced against **real providers** (Go refs via local-process + the SQLite catalog as a container under the podman full-I9 profile — declared caps return real results, undeclared caps the providers implement are denied). **C4** now closes the **stream audit trail**: ADR-008 already audits the stream's open decision; the gateway now emits a **terminal close record** (Outcome success/error/canceled + frames + error, correlation-linked to the open) — a denied stream gets only the deny record. **D1** is the two enforcing runtimes (`local-process` + `podman` full-profile, kernel-verified by `make core-test-podman`). Commits … `4f3854e` · `6e66a24` · `1ba9f18`. **Next on `phase-1` (remaining DoD):** **C3** no-deadline idle-timeout backstop (the gateway bounds by `min(channel, deadline)` already; add the streaming idle-timeout so a silent-but-open provider can't pin a stream forever) · **D3** storage-cred isolation (unblocked by the real process boundary) · **D4** conformance-attestation *enforced* (the podman isolation receipt is the seed) · **D2** real bulk leg · **C1** against real backends · **sre#4** reconciler crash-loop backoff/jitter. CI (`make core-test` + `make core-test-podman` + `make breaking`) green; **schedule Q02** external review; keep the freeze **local/unpushed**.
