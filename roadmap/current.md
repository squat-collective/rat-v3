...
**Definition of done** (the board's exit criteria — [reviews/10](../reviews/10-phase-1-spike-exit.md)): **✅ C5** (extended to real providers — DONE) · **✅ C4** terminal audit incl. denials (DONE) · **✅ C3** (no-deadline idle-timeout backstop — DONE) · **✅ D1** a real *enforcing* deployment-runtime (podman, full I9 profile — DONE) · **D2** (wire into the real bulk leg) · **✅ D3** storage-cred isolation (DONE) · **✅ D4** conformance-attestation *enforced* (DONE) · **C1** (against real backends) · **sre#4** reconciler crash-loop backoff/jitter.

**Immediate next concrete step:** 🎯 **7 of 9 Phase-1 exit criteria cleared.** Done: the gateway **C-series** — **C5** (capability enforcement, against real providers) · **C4** (audit every decision + terminal stream-close) · **C3** (deadline bound + no-deadline idle backstop) · **C1** (crash-safety idempotency); the deployment **D-series so far** — **D1** (two enforcing runtimes: `local-process` + `podman` full-I9, kernel-verified) · **D3** (storage-cred scoping: tenant-isolated, contained, through the gateway) · **D4** (conformance attestation: the core verifies `declared == conformed` via signed ed25519 attestations — `registry.NewVerified`; the core's first real signature verification). Recent commits `7a8b386` (D3) · `9e7edca` (D4). **Next on `phase-1` — the last two DoD items:** **D2** real bulk leg (the Arrow bytes plane — `ArrowStream` ticket TTL / single-use / `{stream,caller,tenant}`-binding, vector-tested) · **sre#4** reconciler crash-loop backoff + jitter + lease-thrash guard. Then the Phase-1 acceptance criteria are met → consider the `phase-1` → `main` seal (`rat/2.0`). CI (`make core-test` + `make core-test-podman` + `make breaking`) green; **schedule Q02** external review; keep the freeze **local/unpushed**.
