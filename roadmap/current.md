...
**Immediate next concrete step:** ✅ **The C5 spine is built + tested end-to-end** — `core/registry` (`Authorize` derived from declared `requires`/`provides`) + `core/gateway` (the `CapabilityInvokeService`: enforces that decision per call, audits it (C4), relays opaque frames; allow / deny-undeclared / deny-unknown-caller / stream-deny-at-open / C1-reject all `go test`-green against a real gRPC provider — commits `fdcf780`, `de34989`). **Next on `phase-1`:** (1) **composition-on-Go** — re-run the real pipeline (catalog→engine→format→strategy) through this gateway with a manifest per plugin, asserting the same target the Python composition produces — *this is where a frozen-wire insufficiency would surface = a freeze-reopen while local*; (2) the **C1 crash-mid-strategy** + **C2 truncation** cases; (3) **wire CI** (`go test ./core/...` + `buf breaking` + the make gates). Keep the freeze **local/unpushed** (no remote, no BSR) until C5 enforcement passes against the composition.
