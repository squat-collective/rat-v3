...
**Immediate next concrete step:** ✅ **D1 steps 1–2 DONE** — the `local-process` deployment-runtime (`core/deploymentruntime`) + the **supervisor** (`core/supervisor`: manifests → `Launch` → healthcheck → dial → register → gateway). The gateway now routes C5-authorized calls to **launched, isolated child processes** (proven via `stateplugin` — distinct PID; commits `c638202` · `61be935`). **Next on `phase-1`:** (3) promote the in-test catalog/format fakes to standalone binaries; (4) **re-run composition-on-Go through launched providers** (the full catalog→format→catalog pipeline via the supervisor — distinct PIDs, C5 + C1 still enforced). Then the **podman** runtime for the full I9 profile (read-only-fs / metadata-egress) = **D1 complete**. CI (`make core-test` + `make breaking`) green; **schedule Q02** external review; keep the freeze **local/unpushed**.
