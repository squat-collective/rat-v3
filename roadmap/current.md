...
**Immediate next concrete step:** ✅ **The C5 spike's centerpiece is DONE + green** — `core/{manifest,registry,gateway,composition}`: the enforcing gateway mediates the real cross-axis pipeline (catalog `get-table` → format `overwrite` → catalog `commit-table`) with C5 derived from manifests + audited (C4), and **crash-mid-strategy recovers with no double-apply** (C1). **🔑 Finding:** the frozen wire *sufficed* — the strategy axis did **not** need a commit/abort shape for crash-between-write-and-commit recovery (existing `idempotency_key`/`already_applied`, ADR-012). **No freeze-reopen.** (`go build`+`vet`+`test ./core/...` green; commits `fdcf780` · `de34989` · `dfd6587`.) **Remaining to close the spike:** (1) the lighter probes **C3** (provider-call deadline on the bytes leg) + **D2** (ArrowStream-ticket TTL/single-use/binding); (2) **wire CI** (`make core-test`); (3) the **spike exit report** → which feeds Tom's deferred 12–18mo commitment-gate decision ([ADR-013](../docs/architecture/adrs/013-phase-1-spike-and-commitment-gate.md)). The **full** Phase-1 definition-of-done (C4-terminal, D1 real isolation, D3, D4 attestation, sre#4 backoff) is the post-spike core build. Keep the freeze **local/unpushed** until then.
