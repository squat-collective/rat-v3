...
**Immediate next concrete step:** 🎉 **D1 COMPLETE** — both deployment-runtimes are real: `local-process` (process-level I9 subset) + **`podman` (full I9 profile, kernel-enforced)**. The supervisor brings launched, isolated providers up behind the gateway and the cross-axis composition runs through them (distinct PIDs; C5 + C1 + commit-linkage held). `core/deploymentruntime.Podman` maps the `IsolationProfile` 1:1 onto podman flags (`--user`/`--cap-drop=ALL`/`--security-opt=no-new-privileges`/`--read-only`/seccomp) + a private `--network=bridge` netns, and emits a structured isolation receipt. The **live vector** (`make core-test-podman`: a privileged go+podman container drives a real nested `podman run` + an in-container prober) proves the kernel enforced all five controls — uid≠0, CapEff=0, NoNewPrivs=1, read-only root (EROFS), `169.254.169.254` unreachable — closing the reviews/08 D1 honesty gap. Commits `c638202` · `61be935` · `c37ce7b` · `4f3854e`. **Next on `phase-1` (remaining DoD):** the real process boundary now unblocks **C5 against real providers** + **D3** storage-cred isolation ([ADR-016](../docs/architecture/adrs/016-plugin-provisioning-via-deployment-runtime.md)); plus **C4** terminal audit, **C3** idle-timeout backstop, **D2** real bulk leg, **D4** conformance-attestation *enforced* (the structured receipt is the seed), **C1** against real backends, **sre#4** reconciler backoff/jitter. CI (`make core-test` + `make core-test-podman` + `make breaking`) green; **schedule Q02** external review; keep the freeze **local/unpushed**.
