...
- **Q02 — external peer review** (still owed per [reviews/09](../reviews/09-phase-1-gate-review.md) / [ADR-013](../docs/architecture/adrs/013-phase-1-spike-and-commitment-gate.md)): the spike/core has only had internal adversarial review; get outside eyes on the architecture before broad commitment. The freeze stays **local/unpushed** until then. ✅ **Recruiting kit drafted** → [brief](../reviews/Q02-external-review-brief.md) + [outreach note](../reviews/Q02-outreach-note.md); **next: recruit reviewers** (OSGi/K8s/VSCode/Temporal-class) + run it.
