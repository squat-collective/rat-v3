...
([ADR-051](../docs/architecture/adrs/051-publish-apache2-squat-collective.md); license/org/visibility
chosen by Tom):
...
- **CI repaired before it ever ran:** `contracts.yml`'s SDK job regenerated FOUR languages
  ...
  `ghcr.io/squat-collective/rat-v3-plugin-base-{go,py}`** (the SDK distribution — Tom's "defer PyPI,
...
- **Pushed public** (`main` + the `rat/6.18` tag only — each pushed tag cuts a Release, so
  ...
  module, `podman pull ghcr.io/squat-collective/rat-v3`.
