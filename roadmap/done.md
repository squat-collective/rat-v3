...
## 2026-06-02 — Q02 kit extended: SRE / operability-focused brief

[`reviews/Q02-brief-sre.md`](../reviews/Q02-brief-sre.md), on `phase-1-q02-sre`. The fourth Q02 recruiting-kit artifact — a reliability-tailored companion (parallel to the security one) that front-loads the **failure domain + operability questions** for an SRE reviewer: the tier-0 **state-backend SPOF** (its HA = the platform's HA), **diagnosability** across polyglot plugins (`rat diagnose`), **native `/metrics` + SLOs** (still paper — sre#8), the **single-leader reconcile loop's capacity** + fairness, **upgrade/version-skew**, **DR/backup** consistency, **resource-limit enforcement**, and a failure-mode catalog (bus-down / state-backend-down / eviction storms). States **real-vs-paper up front** — sre#4's crash-loop backoff + lease-thrash guard are DONE; most of [reviews/03](../reviews/03-operations-sre.md)'s recommendations remain open — so the reviewer models the right system. **Q02 kit now:** [brief](../reviews/Q02-external-review-brief.md) + [outreach note](../reviews/Q02-outreach-note.md) + [security brief](../reviews/Q02-brief-security.md) + [SRE brief](../reviews/Q02-brief-sre.md). Next: recruit reviewers + run it.
