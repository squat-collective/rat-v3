...
- **The remote-access + security architecture** it sits on — the **shared-responsibility model** (perimeter environmental · identity+C5+audit core · authn/connectivity pluggable) — explored in depth; **owed its own ADR** (the security-responsibility-model ADR), captured in conversation. The `connectivity` axis (reverse-tunnel/mesh/**nats-leaf**) was demoted from a security mechanism to an optional convenience.
