...
2. **Identity-axis reference plugin + TLS + the binding guardrail** (ADR-034 Q01/Q02, ADR-033 Q03) — *now the active next step.* A real identity plugin (API token → OIDC), TLS on the listener, and the secure-by-default check (public bind ⇒ require TLS + identity, else fail-loud) in `rat serve`/`rat hub`. Closes the trust-asserted `--as` gap *at the edge*; the prerequisite for any public bind / public hub.
