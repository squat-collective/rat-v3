...
- **`format` = REAL pair — remaining (the last round-2 axis)**: option (b) — **parquet** (pyarrow) + **delta** (`deltalake`) writing real Arrow data files, sharing a `format-real-v1.json`. The heaviest: real file IO + the Append/Merge/Overwrite/Resolve data lifecycle on real files (the `streams.py` Arrow-IPC pattern from engine carries over). Completes round 2 (6/6).
