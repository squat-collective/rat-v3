...
- **`engine` = duckdb** + **`format` = parquet/iceberg** — remaining, and NOT drop-in. Their round-1 conformance data is provider-specific: the engine vectors are toy-mini-SQL (untyped — real DuckDB needs typed SQL, which the toy regex parser can't run), and the format bulk leg is an in-process stream-registry stand-in in all refs (a real backend forces the real Arrow Flight data leg — the deferred typed-Arrow pass). Each needs a DESIGN DECISION first: (a) rework the conformance vectors to be provider-neutral across toy + real backends, or (b) replace the toy pair with two REAL backends (duckdb + datafusion; parquet + iceberg) per ADR-003's original intent. Larger than the other four; surfaced 2026-05-31.
