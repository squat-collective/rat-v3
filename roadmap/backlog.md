...
- ~~DX-2~~ — **done at `rat/6.18`** ([ADR-051](../docs/architecture/adrs/051-publish-apache2-squat-collective.md)): published public at `github.com/squat-collective/rat-v3` under Apache-2.0; releases ship 4-platform binaries + install.sh + the daemon and plugin-base images on GHCR; the Go SDK is `go get`-able (module paths renamed `rat-dev`→`squat-collective/rat-v3` with the publish).
- **DX-2-residual (Low)** — **PyPI packaging** for the Python SDK (pyproject + a PyPI account/token; deferred by choice — the SDK ships in `ghcr.io/squat-collective/rat-v3-plugin-base-py` meanwhile). Also: the historical `rat/1.5`–`6.17` tags are local-only (each pushed tag cuts a Release — push deliberately or not at all), and `marketplace.go`'s official-index URL now points at `squat-collective.github.io/marketplace` (still a placeholder until that Pages site exists).
