...
| 0d | Reference implementations round 1 — 1 per critical axis (6 plugins) | 6-8 weeks, parallelizable — **`format` round-1 ✅ (`inmemory-go`)** |
| 0e | Reference implementations round 2 — 1 per critical axis (6 more) | 3-5 weeks — **`format` round-2 ✅ (`inmemory-py`); shared golden vectors + stub ADR-005 gateway landed → `format/v1` ADR-003 two-reference gate MET (blocked on identity-transport ADR + typed-Arrow pass before `v1`)** |
