...
| 0d | Reference implementations round 1 — 1 per critical axis (6 plugins) | 6-8 weeks, parallelizable — **`format`✅ `engine`✅ `storage`✅ `runtime`✅ `catalog`✅ round-1 (`inmemory-go` each); 5 of 6 data-plane axes done (only `state` left). NOTE: "round 1/2" here = the two LANGUAGE twins (wire-contract gate); the technologically-divergent real-backend pair is a separate round, tracked in backlog.** |
| 0e | Reference implementations round 2 — 1 per critical axis (6 more) | 3-5 weeks — **`format`✅ `engine`✅ `storage`✅ `runtime`✅ `catalog`✅ round-2 (`inmemory-py` each); shared golden vectors → all five axes' WIRE-CONTRACT two-reference gate MET. ADR-007 (identity transport) + ADR-008 (streaming invoke) both decided AND migrated; storage reads tenant from envelope; runtime gateway-mediated via InvokeServerStream; catalog exercises merge-safety (CAS + idempotency). Remaining: `state`, then round-2 real backends + typed-Arrow before `v1`.** |
