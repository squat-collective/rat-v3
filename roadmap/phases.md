...
> **🎯 The product — [ADR-020](../docs/architecture/adrs/020-data-platform-bundle.md) the data platform bundle (Accepted, 2026-06-02).** A single `platform/` folder assembling the core + reference plugins into a usable platform: a **landing zone** (raw CSV) → **medallion** (bronze→silver→gold) of editable SQL/Python models → **data-quality tests**, run through `rat serve` and edited via `vscode-rat` + `ratctl`. The v2 product (`ratatouille-v2`) rebuilt on v3, web portal → **VS Code + CLI**. Build order: **M1** scaffold + a *local* medallion demo · **M2** containerize (attach-mode `compose up` + Postgres/MinIO) · **M3** data-quality tests · **M4** VS Code. Mostly assembly (engine/catalog/storage/ui/cli/serve exist); new = the medallion/landing/quality *conventions* + a thin transform runner + attach mode.

> **Status: Phase 2 IN-FLIGHT.** ✅ ADR-019 Phase A (the core runs as a server: `rat serve`, `make core-serve-smoke`) · ✅ containerized daemon (`core/Dockerfile`, `make rat-image`) · ✅ `ratctl` (a client drives the orchestrator, `make ratctl-smoke`). **NOW: ADR-020 M1** — scaffold `platform/` + a local medallion demo.
