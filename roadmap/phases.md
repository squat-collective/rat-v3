...
- **D2/✅ D3** bytes-plane isolation — ArrowStream-ticket (TTL/single-use/binding) + storage-cred scoping are vector-tested, not honor-system. **D3 DONE (2026-06-01):** the real `localfs-go` storage ref, launched behind the gateway, vends creds scoped to (tenant, prefix, mode, short TTL) with per-tenant root isolation + path containment (escape → `PERMISSION_DENIED`); tenant is read only from the gateway-re-stamped envelope. **D2** (ArrowStream-ticket on the real bulk leg) remains.
