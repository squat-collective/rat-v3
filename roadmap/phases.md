...
| 0f | Conformance suites + per-RPC latency benchmark | 3 weeks — **conformance suite runner ✅ DONE (`make conformance` → 20/20 references conform; auto-discovers refs, one pass/fail matrix, CI/freeze-gateable). Real Arrow Flight transport ✅ (parquet-py). Remaining: per-RPC latency benchmark (optional).** |
