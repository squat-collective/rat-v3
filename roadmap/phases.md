...
| 0e | Reference implementations round 2 — 1 per critical axis (6 more) | 3-5 weeks — **`format` ✅ + `engine` ✅ + `storage` ✅ + `runtime` ✅ round-2 (`inmemory-py` each); shared golden vectors → all four axes' ADR-003 two-reference gate MET. Identity-transport resolved (ADR-007, migrated); storage reads tenant from envelope; runtime is first streaming axis + surfaced the unary-Invoke streaming-mediation gap (candidate ADR). Remaining before `v1`: typed-Arrow pass + streaming-invoke. Axes left: catalog, state.** |
