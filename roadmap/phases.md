...
| 0d | Reference implementations round 1 — 1 per critical axis (6 plugins) | 6-8 weeks, parallelizable — **`format` ✅ + `engine` ✅ + `storage` ✅ + `runtime` ✅ round-1 (`inmemory-go` each); 4 of 6 data-plane axes done** |
| 0e | Reference implementations round 2 — 1 per critical axis (6 more) | 3-5 weeks — **`format` ✅ + `engine` ✅ + `storage` ✅ + `runtime` ✅ round-2 (`inmemory-py` each); shared golden vectors → all four axes' ADR-003 two-reference gate MET. Identity-transport resolved (ADR-007, migrated); storage reads tenant from envelope; runtime is first streaming axis — surfaced AND resolved the streaming-mediation gap (ADR-008, migrated: `InvokeServerStream`, runtime now gateway-mediated). Remaining before `v1`: typed-Arrow pass. Axes left: catalog, state.** |
