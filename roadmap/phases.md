...
| 0e | Reference implementations round 2 — 1 per critical axis (6 more) | 3-5 weeks — **✅ COMPLETE (language-twin sense): all six axes' `inmemory-py`; shared golden vectors → WIRE-CONTRACT two-reference gate MET; all 12 refs green. ADR-007 (identity transport) + ADR-008 (streaming invoke) decided AND migrated; gateway mediates unary + server-streaming; storage reads tenant; catalog merge-safety; state CAS + key-grammar. STILL PENDING before `v1`: the technologically-divergent REAL-backend round (state=sqlite, storage=local-fs, …) + typed-Arrow — see backlog.** |
