...
> ⚠️ **WIRE-CONTRACT REFERENCE ONLY — NOT A STARTER TEMPLATE.** This round-1 reference validates the `runtime/v1` wire contract (proto shapes, error model, the cross-cutting `rat-callmeta-bin` envelope) with an **in-memory stand-in** — it deliberately fakes things a real plugin must not copy (in-process data stand-ins, ignored hints). For a production-shaped implementation, copy the **round-2 real backend** instead: [`subprocess-py`](../subprocess-py). See [reviews/08](../../../reviews/08-post-freeze-board-review.md) E3.
...
podman run --rm \
  ...
  -w /work/plugins/runtime/inmemory-py \
