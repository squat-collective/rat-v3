"""The SecretService gRPC implementation (Python) — `secret` reference.

Implements Resolve: read the caller's tenant FROM the rat-callmeta-bin metadata
header (ADR-007), then resolve (tenant, secret_ref) against the in-memory store.
Resolution is tenant-scoped and anti-enumeration: an unknown ref AND a ref that
exists only for a DIFFERENT tenant both return found=false + empty value (NOT a
PERMISSION_DENIED status). See the proto's FOUND SEMANTICS comment — this is the
...
class SecretServicer(secret_pb2_grpc.SecretServiceServicer):
    ...
    def Resolve(self, request, context):
        ...
        found, value, expires = self._store.resolve(
            tenant, request.secret_ref, self._now_ms()
        )
        return secret_pb2.ResolveResponse(
            found=found,
            value=value,
