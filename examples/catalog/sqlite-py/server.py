"""The CatalogService gRPC implementation (Python, sqlite-backed) — round-2 reference.

Identical surface to the in-memory catalog references (GetTable / CreateBranch /
MergeBranch); only the store is sqlite-backed. CatalogError → the matching gRPC
status via context.abort. RequestContext is NOT a field (ADR-007); identity ignored.
"""
