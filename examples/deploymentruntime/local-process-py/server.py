"""The DeploymentRuntimeService gRPC implementation (Python) — local-process runtime.

Launch/Terminate/Healthcheck. Launch validates the I9 isolation gate (deployment_
runtime.proto SECURITY) and spawns a child process; Healthcheck reports liveness +
the isolation-honored receipt in `detail`; Terminate stops it. RequestContext is NOT
a field (ADR-007).
"""

from rat.deploymentruntime.v1 import deployment_runtime_pb2, deployment_runtime_pb2_grpc

from store import DeploymentError, LocalProcessRuntime
...
class DeploymentRuntimeServicer(deployment_runtime_pb2_grpc.DeploymentRuntimeServiceServicer):
    def __init__(self, runtime=None) -> None:
        self.rt = runtime or LocalProcessRuntime()
