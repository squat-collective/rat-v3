...
pub mod secret_service_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        > {
            ...
            req.extensions_mut()
                .insert(GrpcMethod::new("rat.secret.v1.SecretService", "Resolve"));
...
pub mod secret_service_server {
    ...
    #[async_trait]
    pub trait SecretService: std::marker::Send + std::marker::Sync + 'static {
        async fn resolve(
            &self,
    ...
    #[derive(Debug)]
    pub struct SecretServiceServer<T> {
        inner: Arc<T>,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/rat.secret.v1.SecretService/Resolve" => {
                    ...
                    struct ResolveSvc<T: SecretService>(pub Arc<T>);
                    impl<
                        T: SecretService,
                    ...
                    for ResolveSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as SecretService>::resolve(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for SecretServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "rat.secret.v1.SecretService";
    impl<T> tonic::server::NamedService for SecretServiceServer<T> {
